...
import { DbReview, Franchise } from "@/types";
import { OFFICIAL_WA_LINK } from "@/lib/constants";
...
export default function FranchiseDetailContent({ initialFranchise, initialReviews }: Props) {
    ...
    const initClientData = async () => {
      ...
      if (currentUserId) {
        const { data: savedData } = await supabase
          ...
          .eq('franchise_id', initialFranchise.id)
          .single();
      ...
      try {
        ...
        await supabase.from('page_views').insert({
          franchise_id: initialFranchise.id,
  ...
  const generateAnalysis = async () => {
    ...
    const prompt = `Berikan analisis strategis mendalam untuk franchise ${initialFranchise.name} dengan data: Investasi ${initialFranchise.invest}, ROI ${initialFranchise.roi}, Omzet ${initialFranchise.omzet}. Berikan analisis SWOT singkat dan rekomendasi keberhasilan. Gaya bahasa: Senior Business Analyst.`;
  ...
  return (
    <main className="pt-28 md:pt-32 pb-20 min-h-screen bg-[#FFF9F0]">
      <div className="max-w-5xl mx-auto px-5">
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-10 items-start">
          <div className="space-y-10 order-2 lg:order-1">
            <ScrollReveal variants={fadeRight}>
              <div className="hidden lg:block">
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  <div className="bg-gray-100 text-gray-600 font-bold text-xs px-3 py-1.5 rounded-full flex items-center gap-1">
                    ...
                    {initialFranchise.rating}
                  </div>
                  <div className="bg-gray-100 text-gray-600 font-bold text-xs px-3 py-1.5 rounded-full flex items-center gap-1">
                    <MapPin className="w-3 h-3" />
                    {initialFranchise.city}
                  </div>
                </div>
                
                <h1 className="font-syne font-extrabold text-5xl text-[#111] leading-[1.1] mb-6">
                  {initialFranchise.name}
                </h1>
                <p className="text-gray-500 text-lg mb-8 leading-relaxed max-w-2xl">
                  Bergabunglah dengan jaringan kemitraan {initialFranchise.name} yang telah sukses di {initialFranchise.city}. Kami menawarkan sistem yang teruji, bahan baku berkualitas, dan dukungan pemasaran berkelanjutan untuk memastikan kesuksesan bisnis Anda.
                </p>
              </div>
              ...
              <div className="lg:hidden">
                ...
                <p className="text-gray-500 mb-6">
                  Bergabunglah dengan jaringan kemitraan {initialFranchise.name} sukses.
                </p>
              </div>
            </ScrollReveal>
            ...
            <StaggerReveal className="grid grid-cols-2 sm:grid-cols-3 gap-4" fast>
              {[
                { label: "Modal Investasi", value: initialFranchise.invest, color: "text-[#111]" },
                { label: "Estimasi ROI", value: initialFranchise.roi, color: "text-[#1B8C5A]" },
                { label: "Jumlah Mitra", value: initialFranchise.mitra, color: "text-[#111]" },
              ].map((s) => (
              ...
              <ScrollReveal variants={fadeRight} delay={0.1}>
                <div className="bg-white rounded-[32px] p-8 border border-black/5 shadow-sm hover:shadow-lg transition-shadow duration-300">
                  ...
                  <ul className="space-y-4">
                    {[
                      { label: "Estimasi Omzet / Bulan", value: initialFranchise.omzet },
            ...
            <aside className="space-y-6">
              <div className="bg-white rounded-[32px] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-black/5 relative overflow-hidden group">
                <div className="aspect-[4/3] relative rounded-2xl overflow-hidden mb-6 shadow-inner bg-gray-100">
                  <Image 
                    src={initialFranchise.img || ""}
                    alt={initialFranchise.name}
                    fill
              ...
              <div className="bg-[#111] rounded-[24px] p-6 text-white overflow-hidden relative">
                ...
                <Link 
                  href={`${OFFICIAL_WA_LINK}?text=Halo%20EazyChise,%20saya%20tertarik%20dengan%20franchise%20${initialFranchise.name}`}
                  target="_blank"
        ...
        <LocationRecommender category={initialFranchise.cat || ""} />
        ...
        >
          {!analysis && !isAnalyzing ? (
            ...
            >
              ...
              Dapatkan Analisis Strategis AI untuk {initialFranchise.name}
            </button>
          ) : (
        ...
        <ScrollReveal variants={fadeUp} className="mt-16 bg-white rounded-[2.5rem] p-8 md:p-12 shadow-xl border border-black/5">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2 bg-[#F8F8F6] px-4 py-2 rounded-full">
              ...
              <span className="font-bold text-[#111]">{initialFranchise.rating}</span>
                    ...
                    <div key={rev.id} className="bg-gray-50 p-5 rounded-2xl border border-black/5">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="font-bold text-sm">Mitra {rev.user_id ? "Verifikasi" : "Anonim"}</p>
                        </div>
        ...
        <ScrollReveal variants={fadeUp} delay={0.1} className="mt-16">
          <div className="bg-[#111] rounded-[32px] p-10 text-white flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden relative">
            ...
            <div className="relative z-10 text-center md:text-left">
              <h2 className="font-syne font-extrabold text-3xl mb-2 text-white">Siap Mulai Bisnis {initialFranchise.name}?</h2>
      ...
      <PartnershipModal 
        ...
        franchiseName={initialFranchise.name} 
        franchiseId={initialFranchise.id}
      />
    </main>
  );
}
